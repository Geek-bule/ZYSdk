...
- (void)hide:(BOOL)animated afterDelay:(NSTimeInterval)delay withTip:(NSString *)tip {
    [self performSelector:@selector(hideView:) withObject:tip afterDelay:delay];
}

- (void)hideView:(NSString*) tip
{
    self.labelText = @"Timeout!";
    self.detailsLabelText = tip;
    [self hide:YES afterDelay:4.0];
}
