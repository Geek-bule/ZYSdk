...
- (id)init{
    ...
    if (self) {
        ...
        _isShowLog = NO;
        //appid
        self.strAppID = NSLocalizedString(@"APP_ID", nil);
        //设置回调
...
- (BOOL)isCanRateApp
{
    ...
    NSString* rate = [[ZYParamOnline shareParam] getParamOf:@"rate"];
...
- (void)RateWithTipAndBlock:(rateBack)rateCall
{
    ...
    NSString* hide = [[ZYParamOnline shareParam] getParamOf:@"ratehide"];
    if (hide.intValue == 1) {
